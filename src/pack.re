...
let formatBundle = (registerRequire, base, modules) => {
  {|
...
let nameMap = {
  |} ++
  (List.map(
    ((id, path, body)) => "\"" ++ String.escaped(path) ++ "\": " ++ string_of_int(id),
    modules
  ) |> String.concat(",\n  "))
  ++ {|
}
...
let process = (~externalEverything=false, ~renames, ~base=Unix.getcwd(), abspath) => {
  let state = {
    entry: abspath,
    externalEverything,
    alias: mapOf(renames),
  ...
  formatBundle(!state.externalEverything, base, state.modules)
};